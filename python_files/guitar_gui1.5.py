...
def generate_music():
    ...
    gen_win.geometry("300x80")
    
    temp_label = tk.Label(gen_win, text="Temperature")
    temp_entry = tk.Entry(gen_win, width=4)
    length_label = tk.Label(gen_win, text="Length of generation (notes)")
    length_entry = tk.Entry(gen_win, width=4)

    if (temperature != None) and (temp_entry != None):
        length_entry.insert(0, str(length))
        temp_entry.insert(0, str(temperature))
    
    length_label.place(relx=0, rely=0.22)
    temp_entry.place(relx=0.3, rely=0)
    temp_label.place(relx=0, rely=0)
    length_entry.place(relx=0.55, rely=0.22)
    conf_button = tk.Button(gen_win, text="Confirm Values", command=lambda: set_variables()).place(relx=0.05, rely=0.5)
...
generate_b = tk.Button(window, text="Generate Music", command=lambda: generate_music())
